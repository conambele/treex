...
sub process_tnode {
    ...
    return if !(any { $_->tag =~ /^[23]/ } $t_node->get_anodes());
    ...
    # passive plural
    elsif ( any { $_ =~ /^3..[JKL]...[456]/ } @anode_tags ) { # include 'tu', 'vos'
        $number = 'sg';
        $politeness = 'basic';
        $should_add = 0 if $t_node->t_lemma !~ /r$/;
    }
    # active singular
    elsif ( any { $_ =~ /^3..[ABC]...[456]/ } @anode_tags ) { # include 'tu', 'vos'
        $politeness = 'basic';
        $number = 'sg';
    }
